...
async def save_single_file(bot, message):
    ...
    if not reply.media:
        return await message.reply(
            "Please reply to a media message (video/audio/document) to save it."
        )

    if reply.media not in [
        enums.MessageMediaType.VIDEO,
        enums.MessageMediaType.AUDIO,
        enums.MessageMediaType.DOCUMENT,
    ]:
        return await message.reply(
            "Unsupported media type. Only video, audio and documents are supported."
        )

    msg = await message.reply(
        "<i>Checking media and saving to database...</i>", quote=True
    )
    ...
    try:
        ...
        if saved:
            ...
            await msg.edit(
                "❌ <b>Error occurred while saving file to database.</b>\n\n<i>Check logs for details.</i>"
            )
