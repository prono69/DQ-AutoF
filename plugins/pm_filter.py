...
from database.gfilters_mdb import del_allg, find_gfilter, get_gfilters
from database.ia_filterdb import (
    Media,
    Media2,
)
from database.ia_filterdb import db as clientDB
from database.ia_filterdb import db2 as clientDB2
from database.ia_filterdb import (
    get_bad_files,
    get_file_details,
    get_search_results,
...
async def next_page(bot, query):
    ...
    btn.insert(
        ...
        [
            ...
            InlineKeyboardButton(
                "! Lᴀɴɢᴜᴀɢᴇs !", callback_data=f"select_lang#{req}"),
        ],
    )
    btn.insert(
        0,
...
async def select_language(bot, query):
    ...
    btn = [
        ...
        [
            InlineKeyboardButton(
                "Eɴɢʟɪꜱʜ", callback_data=f"lang#{userid}#eng"),
            InlineKeyboardButton("Tᴀᴍɪʟ", callback_data=f"lang#{userid}#tam"),
            InlineKeyboardButton("Hɪɴᴅɪ", callback_data=f"lang#{userid}#hin"),
        ],
        [
            InlineKeyboardButton(
                "Kᴀɴɴᴀᴅᴀ", callback_data=f"lang#{userid}#kan"),
            InlineKeyboardButton("Tᴇʟᴜɢᴜ", callback_data=f"lang#{userid}#tel"),
        ],
        [InlineKeyboardButton(
            "Mᴀʟᴀʏᴀʟᴀᴍ", callback_data=f"lang#{userid}#mal")],
        [
            InlineKeyboardButton(
                "Mᴜʟᴛɪ Aᴜᴅɪᴏ", callback_data=f"lang#{userid}#multi"),
            InlineKeyboardButton(
                "Dᴜᴀʟ Aᴜᴅɪᴏ", callback_data=f"lang#{userid}#dual"),
        ],
...
async def cb_handler(client: Client, query: CallbackQuery):
    ...
    if query.data.startswith("file"):
        ...
        btn2 = [[InlineKeyboardButton(
            "Vɪᴇᴡ Sᴛᴀᴛᴜs", url=f"{query.message.link}")]]
        ...
        if int(query.from_user.id) == int(from_user):
            user = await client.get_users(from_user)
            await query.answer(
                f"Hᴇʏ {user.first_name}, Yᴏᴜʀ Rᴇᴏ̨ᴜᴇsᴛ ɪs Uɴᴀᴠᴀɪʟᴀʙʟᴇ !", show_alert=True
            )
        else:
            await query.answer(
                "Yᴏᴜ ᴅᴏɴ'ᴛ ʜᴀᴠᴇ sᴜғғɪᴄɪᴀɴᴛ ʀɪɢʜᴛs ᴛᴏ ᴅᴏ ᴛʜɪs !", show_alert=True
        ...
        buttons = [
            [
                ...
                InlineKeyboardButton(
                    "Aᴜᴛᴏ FIʟᴛᴇʀ", callback_data="autofilter"),
            ],
            [
        ...
        buttons = [[InlineKeyboardButton(
            "⟸ Bᴀᴄᴋ", callback_data="manuelfilter")]]
        reply_markup = InlineKeyboardMarkup(buttons)
        await query.message.edit_text(
    ...
    if imdb and imdb.get("poster"):
        try:
            ...
            hmm = await message.reply_photo(
                photo=poster, caption=cap[:1024], reply_markup=InlineKeyboardMarkup(
                    btn)
            )
            try:
                if settings["auto_delete"]:
            ...
            fek = await message.reply_photo(
                photo=NOR_IMG, caption=cap, reply_markup=InlineKeyboardMarkup(
                    btn)
            )
            try:
                if settings["auto_delete"]:
                    ...
                    if fileid == "None":
                        if btn == "[]":
                            return await client.send_message(
                                ...
                                disable_web_page_preview=True,
                                protect_content=(
                                    True if settings["file_secure"] else False
                                ),
