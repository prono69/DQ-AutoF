...
from database.caption_db import get_caption_template, set_caption_template
        ...
        if is_valid == True:
            if fileid == "send_all":
                btn = [
                    [
                        InlineKeyboardButton(
                            "Gᴇᴛ Fɪʟᴇ", callback_data=f"checksub#send_all"
                        )
                    ]
                ]
    ...
    await client.send_cached_media(
        ...
        reply_markup=InlineKeyboardMarkup(
            [
                ...
                [InlineKeyboardButton("Bᴏᴛ Oᴡɴᴇʀ", url="t.me/kirito6969")],
            ]
        ),
    )
...
async def requests(bot, message):
            ...
            if REQST_CHANNEL is not None:
                btn = [
                    [
                        ...
                        InlineKeyboardButton(
                            "Sʜᴏᴡ Oᴘᴛɪᴏɴs", callback_data=f"show_option#{reporter}"
                        ),
                    ]
                ]
                ...
                if len(content) < 3:
                    await message.reply_text(
                        "<b>Yᴏᴜ ᴍᴜsᴛ ᴛʏᴘᴇ ᴀʙᴏᴜᴛ ʏᴏᴜʀ ʀᴇᴏ̨ᴜᴇsᴛ [Mɪɴɪᴍᴜᴍ 3 Cʜᴀʀᴀᴄᴛᴇʀs]. Rᴇᴏ̨ᴜᴇsᴛs ᴄᴀɴ'ᴛ ʙᴇ ᴇᴍᴘᴛʏ.</b>"
                    )
            if len(content) < 3:
                success = False
        except Exception as e:
        ...
        try:
            if REQST_CHANNEL is not None and len(content) >= 3:
                btn = [
                    [
                        InlineKeyboardButton(
                            "Vɪᴇᴡ Rᴇᴏ̨ᴜᴇsᴛ", url=f"{message.link}"),
                        InlineKeyboardButton(
                            "Sʜᴏᴡ Oᴘᴛɪᴏɴs", callback_data=f"show_option#{reporter}"
                        ),
                    ]
    ...
    if success:
        btn = [[InlineKeyboardButton(
            "Vɪᴇᴡ Rᴇᴏ̨ᴜᴇsᴛ", url=f"{reported_post.link}")]]
        await message.reply_text(
            "<b>Yᴏᴜʀ ʀᴇᴏ̨ᴜᴇsᴛ ʜᴀs ʙᴇᴇɴ ᴀᴅᴅᴇᴅ! Pʟᴇᴀsᴇ ᴡᴀɪᴛ ғᴏʀ sᴏᴍᴇ ᴛɪᴍᴇ.</b>",
            reply_markup=InlineKeyboardMarkup(btn),
        )
...
async def send_msg(bot, message):
    if message.reply_to_message:
        ...
        try:
            ...
            if success:
                await message.reply_text(
                    f"<b>Yᴏᴜʀ ᴍᴇssᴀɢᴇ ʜᴀs ʙᴇᴇɴ sᴜᴄᴄᴇssғᴜʟʟʏ sᴇɴᴅ ᴛᴏ {user.mention}.</b>"
                )
            else:
                await message.reply_text(
                    "<b>Tʜɪs ᴜsᴇʀ ᴅɪᴅɴ'ᴛ sᴛᴀʀᴛᴇᴅ ᴛʜɪs ʙᴏᴛ ʏᴇᴛ!</b>"
                )
        except Exception as e:
            await message.reply_text(f"<b>Eʀʀᴏʀ: {e}</b>")
    else:
        await message.reply_text(
            "<b>Usᴇ ᴛʜɪs ᴄᴏᴍᴍᴀɴᴅ ᴀs ᴀ ʀᴇᴘʟʏ ᴛᴏ ᴀɴʏ ᴍᴇssᴀɢᴇ ᴜsɪɴɢ ᴛʜᴇ ᴛᴀʀɢᴇᴛ ᴄʜᴀᴛ ɪᴅ. Fᴏʀ ᴇɢ: /send ᴜsᴇʀɪᴅ</b>"
        )
...
async def deletemultiplefiles(bot, message):
    ...
    if chat_type != enums.ChatType.PRIVATE:
        return await message.reply_text(
            f"<b>Hᴇʏ {message.from_user.mention}, Tʜɪs ᴄᴏᴍᴍᴀɴᴅ ᴡᴏɴ'ᴛ ᴡᴏʀᴋ ɪɴ ɢʀᴏᴜᴘs. Iᴛ ᴏɴʟʏ ᴡᴏʀᴋs ᴏɴ ᴍʏ PM!</b>"
        )
    else:
        pass
    try:
    ...
    btn = [
        [
            InlineKeyboardButton(
                "Yᴇs, Cᴏɴᴛɪɴᴜᴇ !", callback_data=f"killfilesdq#{keyword}"
            )
        ],
        [InlineKeyboardButton("Nᴏ, Aʙᴏʀᴛ ᴏᴘᴇʀᴀᴛɪᴏɴ !",
...
async def shortlink(bot, message):
    ...
    if chat_type == enums.ChatType.PRIVATE:
        return await message.reply_text(
            f"<b>Hᴇʏ {message.from_user.mention}, Tʜɪs ᴄᴏᴍᴍᴀɴᴅ ᴏɴʟʏ ᴡᴏʀᴋs ᴏɴ ɢʀᴏᴜᴘs !</b>"
        )
    elif chat_type in [enums.ChatType.GROUP, enums.ChatType.SUPERGROUP]:
    ...
    if (
        ...
        return await message.reply_text(
            "<b>Yᴏᴜ ᴅᴏɴ'ᴛ ʜᴀᴠᴇ ᴀᴄᴄᴇss ᴛᴏ ᴜsᴇ ᴛʜɪs ᴄᴏᴍᴍᴀɴᴅ !</b>"
        )
    else:
        pass
    try:
...
async def stop_button(bot, message):
    msg = await bot.send_message(
        text="**𝖡𝗈𝗍 𝖨𝗌 𝖱𝖾𝗌𝗍𝖺𝗋𝗍𝗂𝗇𝗀...🪄**", chat_id=message.chat.id
    )
...
async def stats(bot, update):
    currentTime = time.strftime(
        "%Hh%Mm%Ss", time.gmtime(time.time() - BOT_START_TIME))
...
@Client.on_message(filters.command("viewcap") & filters.admin)
