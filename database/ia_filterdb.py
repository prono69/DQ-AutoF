...
async def save_file(media):
    ...
    try:
        if await Media.count_documents({"file_id": file_id}, limit=1):
            logger.warning(
                f'{getattr(media, "file_name", "NO_FILE")} is already saved in primary DB !'
            )
            return False, 0
        file = saveMedia(
            file_id=file_id,
...
async def get_search_results(
    ...
    if (
        max_results % 2 != 0
    ):  # if max_results is an odd number, add 1 to make it an even number
        logger.info(
    ...
    if len(fileList2) < max_results:
        ...
        cursor.skip(cursorSkipper if cursorSkipper >= 0 else 0).limit(
            max_results - len(fileList2)
        )
